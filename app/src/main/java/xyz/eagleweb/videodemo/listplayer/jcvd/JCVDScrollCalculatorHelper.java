...
public class JCVDScrollCalculatorHelper {
    ...
    public JCVDScrollCalculatorHelper(int playId, int rangeTop, int rangeBottom) {
        this.playId = playId;
    ...
    void playVideo(RecyclerView view) {
        ...
        JzvdStd gsyBaseVideoPlayer = null;

        boolean needPlay = false;

        for (int i = 0; i < visibleCount; i++) {
            if (layoutManager.getChildAt(i) != null && layoutManager.getChildAt(i).findViewById(playId) != null) {
                JzvdStd player = (JzvdStd) layoutManager.getChildAt(i).findViewById(playId);
                Rect rect = new Rect();
                ...
                if (rect.top == 0 && rect.bottom == height) {
                    ...
                    if ((player.currentState == JzvdStd.CURRENT_STATE_NORMAL
                            || player.currentState == JzvdStd.CURRENT_STATE_ERROR)) {
                        needPlay = true;
                    }
                    break;
                }
        ...
        if (gsyBaseVideoPlayer != null && needPlay) {
            if (runnable != null) {
                JzvdStd tmpPlayer = runnable.gsyBaseVideoPlayer;
    ...
    private class PlayRunnable implements Runnable {
        ...
        JzvdStd gsyBaseVideoPlayer;

        public PlayRunnable(JzvdStd gsyBaseVideoPlayer) {
